...
export const findRelevantContent = async (userQuery: string) => {
      ...
      queries: [
        {
          ...
          kNearestNeighborsCount: process.env
  ...
  for await (const result of searchResults.results) {
    const textField = (result.document as any).hasOwnProperty(contentColumn)
      ? (result.document as any)[contentColumn]
      : result.document; // Use specified content field if available, otherwise use document
    const hash = createHash("sha256")
      .update(textField)
      .digest("base64")
      .substring(0, 8); // Create a hash of the text field
